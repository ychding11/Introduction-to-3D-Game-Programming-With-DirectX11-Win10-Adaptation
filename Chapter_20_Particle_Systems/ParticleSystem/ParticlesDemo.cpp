...
bool ParticlesApp::Init()
{
	...
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures\\flare0.dds", nullptr, &mFlareTexSRV));

	mFire.Init(md3dDevice, Effects::FireFX, mFlareTexSRV, mRandomTexSRV, 500); 
	mFire.SetEmitPos(XMFLOAT3(0.0f, 1.0f, 120.0f));

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures\\raindrop.dds", nullptr, &mRainTexSRV ));
